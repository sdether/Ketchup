...
namespace Ketchup.Protocol {
	public abstract class ProtocolException : ApplicationException {
		...
		public ProtocolException(string message, string key, Protocol.Op op, string server)
			: base(string.Format("{0}\nServer Response: {1}\nKey: {2}\nOperation: {3}",message,server,key,op.ToString() )) {
				Key = key;
				Operation = op;
		}
	}
	...
	public class AuthContinueException : ProtocolException {
		public AuthContinueException(string key, Protocol.Op op, string server) :
	...
	public class AuthErrorException : ProtocolException {
		public AuthErrorException(string key, Protocol.Op op, string server) :
	...
	public class BusyException : ProtocolException {
		public BusyException(string key, Protocol.Op op, string server) :
			base("Server too busy.", key, op, server) { }
	}

	public class IncrDecrNonNumericException : ProtocolException {
		public IncrDecrNonNumericException(string key, Protocol.Op op, string server) :
