...
{
	public static class AsyncReplaceExtensions
	{
		...
		public static Bucket Replace<T>(this Bucket bucket, string key, 
			...
			Action<object> success, Action<Exception, object> error, object state) 
		{
			return Replace(bucket, key, value, KetchupConfig.Current.DefaultExpiration, success, error, state);
		}
			...
			Action<object> success, Action<Exception, object> error, object state) 
		{
			...
			return expiration.TotalDays > 30 ?
				Replace(bucket, key, value, DateTime.UtcNow + expiration, success, error, state) :
				Replace(bucket, key, value, Convert.ToInt32(expiration.TotalSeconds), success, error, state);
		}
			...
			Action<object> success, Action<Exception, object> error, object state) 
		{
			...
			return Replace(bucket, key, value, exp, success, error, state);
		}
